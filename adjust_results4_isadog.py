...
def adjust_results4_isadog(results_dic, dogfile):
    ...
    with open(dogfile, "r") as infile:
        ...
        while line != "":
            ...
            if line in dognames_dic:
                print(line, "exists in dictionary")
            else:
                #print(line, "does not exists in dictionary")
    ...
    for key in results_dic:
        ...
        # print(key, "<<<<<<<<key<<<<<<<<<<<")
        # print(results_dic[key][0], "<<<<<<<<<--------Pet Image Label")
        # print(results_dic[key][1], "<<<<<<<<<<<--------Classifier Label in list")
        if results_dic[key][0] in dognames_dic:
            # Classifier Label IS image of Dog (e.g. found in dognames_dic)
            # appends (1, 1) because both labels are dogs
            #print("-----******Pet Image label is a dog.*******------")
            image_label_is_dog = 1
            #results_dic[key].extend((1, 1))
        else:
            #print("-----******Pet Image label is a not dog.*******------")
            image_label_is_dog = 0
            #results_dic[key].extend((1, 0))
   
        classified_is_dog = 0
        lst_classifier = results_dic[key][1].split(",")
        #print("\n")
        #print(results_dic[key][0], ";;", lst_classifier, "lst_classifier----------------------")
        for txt_classifier in lst_classifier:
            txt_classifier = txt_classifier.strip()
            #print(txt_classifier, "<<<<<<<<<<<-----txt Classifier")
            if txt_classifier in dognames_dic:
                classified_is_dog = 1
                #print("Going at break")
                break
        #print(key,",'" , results_dic[key][0] , "'", ", [" ,results_dic[key][1], "], " ,results_dic[key][2], image_label_is_dog, classified_is_dog, "*******")
        results_dic[key].extend([image_label_is_dog, classified_is_dog])
